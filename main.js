...
function startStopwatch() {
    timer = setInterval(function () {
        ...
        screen.textContent = `${pad(hours)}:${pad(minutes)}:${pad(secs)}.${pad(milliseconds)}`;
    }, 1);
}
...
saveBtn.addEventListener('click', function(){
    ...
    savedTime.textContent = screen.textContent;
...
function updateSaveButton(){
    if(seconds === 0 && milliseconds === 0 || isRunning || screen.textContent === '00:00:00'){
        saveBtn.disabled = true;
        saveBtn.style.backgroundColor = 'grey';
    } else if(!isRunning && seconds !== 0 && milliseconds !== 0){
        saveBtn.disabled = false;
        saveBtn.style.backgroundColor = 'blue';
    }
}
